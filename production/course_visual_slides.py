...
def revise(slides):
    ...
    slides[3].update(table=None,image=fig('reconstruction_slide'),layout='image_chart',charts=[chart('trajectory_mse','Mean reconstruction MSE')],caption='Black: recorded; dashed: reconstructed. One median-duration trial. Lower MSE is better. Spline beats each neural model under BH-140; spline vs CAE n=8 does not survive Holm.')
    slides[4].update(body='Context codes establish a personal center. We decode 120 draws around it using shared training covariance.',equation=None,image=fig('generation_slide_n8'),layout='wide_image',caption='Same participant, separate query recordings. n=8, all models. Both capacities appear in the report. Thin: first 30 paths. Thick: mean of all paths. Equal x/y scale. These examples do not determine the cohort ranking.')
    slides[5].update(body='Cumulative curves show individual features. Cohort KS averages all eleven feature discrepancies.',table=None,image=fig('features_selected_n8'),layout='image_chart',charts=[chart('mean_ks','Mean feature KS')],caption='Illustration: subject01, n=8. VAE/CVAE beat spline on all three feature metrics at both capacities (BH and Holm-140). VAE has no uniform significant advantage over CVAE.')
    slides[6].update(body='Energy and MMD compare complete sets of ordered paths, including their variability.',table=None,layout='two_charts',charts=[chart('energy','Full-path energy','raw_rms'),chart('mmd2','Full-path MMD squared','raw_rms')],caption='Lower is better. At n=8, both VAE/CVAE beat spline (BH and Holm-80), also with balanced axes. VAE vs CVAE: q=.672 and 1.000. At n=3, results depend on metric and scaling.')
    ctr=control_means()
    ...
    slides[8].update(body='Stored context resampling has lower mean path discrepancy.\nIt retains complete paths, while the compact models retain a personal center.',caption='No formal test against these references. Reducing 11 correlated features to 9 preserves VAE/CVAE over spline, but weakens a unique VAE claim. No application acceptance threshold was established.')
    ...
    slides[9].update(image=fig('generation_slide_n8'),layout='wide_image',body='VAE n=8 default. Compare n=3, CVAE and spline + PCA. Finish with cohort benchmarks.',caption='Recorded and generated examples if the live demo is unavailable. Same-session variability, with no claim that each latent coordinate represents a named strategy.')
    for s in slides:
        s['backup']=False
