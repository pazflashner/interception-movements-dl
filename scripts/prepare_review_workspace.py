"""Restore dashboard evidence from the newest bundled release; no raw recordings required."""
...
def restore(root: Path, replace: bool = False) -> int:
    root = root.resolve()
    archive = root / "output/release/Interception_Movements_Multimodel_Dashboard.zip"
        ...
        for name in bundle.namelist():
            ...
            if path.startswith("evaluation/") and len(relative.parts) >= 3 and relative.suffix in {".csv", ".json"}:
                relative = PurePosixPath("studies/review_corrected_evaluation/results", *relative.parts[1:])
                      ...
                      or any(path.startswith(f"studies/final_strategy_evaluation/runs/{family}/fold0/") for family in ('cvae','conditional_ae','unconditional_vae'))
