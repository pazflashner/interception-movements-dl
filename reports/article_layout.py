...
def scientific_report(output, tables, fingerprints, figures, example_meta):
    ...
    a.p("The CVAE [3,4] maps a 100 x 2 trajectory and a five-element condition vector (three start-category indicators, side and exact target speed) through two 256-unit ReLU layers to a diagonal-Gaussian posterior. A corresponding two-layer decoder takes a latent draw and the condition vector, and outputs position and two timing channels. Training uses participant-balanced sampling, Adam (learning rate 0.001; batch size 64), gradient clipping at 5, validation early stopping (patience 25) and a 200-epoch limit. All normalization statistics are fitted to training data.")
