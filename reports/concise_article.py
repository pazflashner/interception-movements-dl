...
def scientific_report(output, tables, fingerprints, figures, example_meta):
    ...
    condition_p = tables["condition"].set_index("latent_dim").wilcoxon_p_holm
    a.p(f"The in-sample trajectory K-means analysis gives ARI=0.052 and permutation p=0.00498 (200 permutations), indicating weak non-random structure rather than clean participant clusters. Closed-set enrolment reaches 40.9% at n=3 and 50.7% at n=8 against seven enrolled candidates; this is distinct from generation and cannot establish cross-session stability. The condition-stratum trajectory diagnostic gives Holm-adjusted p={condition_p.loc[3]:.3f} and {condition_p.loc[8]:.3f} at n=3/8, and none of 44 feature-level comparisons survives BH correction. Therefore the condition sliders remain exploratory. Latent-feature heatmaps show within-model associations, not independently validated strategy axes (Appendix S6).")
